...
import { t } from "@/i18n";
import type { ComposerContributionContext } from "./composer-contributions";
/**
 * Exported so the built-in composer-action manifest can declare the renderer
 * without the core composer ever importing this module directly.
 */
export function VoiceInputRender(ctx: ComposerContributionContext) {
  return <VoiceInputButton ctx={ctx} />;
}
...
function VoiceInputButton({ ctx }: { ctx: ComposerContributionContext }) {
  ...
  return (
    ...
    >
      {listening ? (
        <>
          <span className="pointer-events-none absolute inline-flex h-8 w-8 animate-ping rounded-full bg-red-9/20" />
