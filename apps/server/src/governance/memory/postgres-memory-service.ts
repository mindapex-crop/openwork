 ...
 * 原文件：qm/src/memory/postgres-memory-service.ts
 * 移植说明：PG 实现零逻辑改动；仅将 import 路径扩展名从 .ts 改为 .js（../persistence/pg-pool.js、./memory-service.js）以适配 OpenWork NodeNext 模块解析；依赖 ../persistence/pg-pool（由另一个 agent 创建）。
 */

import { createPgPool, withPgTransaction, type PoolClient } from "../../persistence/pg-pool.js";
import { foldCapture, normalizeReplace, queryBullets, recallBody, type MemoryService } from "./memory-service.js";
...
export function createPostgresMemoryService(connectionString: string): MemoryService {
  ...
  async function append(
    ...
    await withPgTransaction(await pool(), async (client: PoolClient) => {
      await client.query("SELECT pg_advisory_xact_lock(hashtext('memory'), hashtext($1))", [scopeId]);
      const head = await client.query(
        "SELECT body, seq FROM memory_revisions WHERE scope_id = $1 ORDER BY seq DESC LIMIT 1",
        [scopeId],
