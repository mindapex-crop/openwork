 ...
 * 原文件：qm/src/security/security-screener.ts
 * 移植说明：移除对 ../util/errors.ts 的 swallow 导入，在文件内本地定义 swallow 辅助函数；保留 createSecurityScreenProxy 的 HTTP proxy 调用层、chunking（1600 字符/chunk，256 重叠）、429 退避重试与 shadow 模式逻辑。
 */
...
function swallow(context: string, error: unknown): void {
  console.warn(`[swallowed] ${context}: ${error instanceof Error ? error.message : String(error)}`);
}
