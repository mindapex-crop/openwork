 ...
 * 原文件：qm/src/acl/acl-store.ts
 * 移植说明：保留 AclStore/AclStoreOptions/GrantPersistence 接口与 createMemoryGrantPersistence/createAclStore 实现；保留 canManage 检查逻辑与"禁止 transitive re-share"语义；保留 handlesForAudience/grantsOfKind 等全部方法。Grant/GrantedHandle 改从 ./types.ts 导入；parseScopeId/samePerson 改从 ../scope.ts 导入（原 QM 中 parseScopeId 来自 ../types.ts、samePerson 来自 ../directory/person.ts）；Principal/ScopeId 改从 ../memory/types.ts 导入。
 */

import type { Grant, GrantedHandle } from "./types.js";
import type { Principal, ScopeId } from "../memory/types.js";
import { parseScopeId } from "../memory/types.js";
import { samePerson } from "../../util/person.js";
import { parseRef, refPrefix } from "./resource-ref.js";
import type { ResourceKind } from "./resource-ref.js";
