 ...
 * 原文件：qm/src/admin/postgres-audit-log.ts
 * 移植说明：保留 CREATE TABLE audit_log + ALTER TABLE + 3 个索引 + 旧表迁移 DO 块；保留 recordOnce 的 ON CONFLICT (idempotency_key) WHERE ... DO NOTHING 幂等；保留 pendingWrites Promise.allSettled 等待；createPgPool 改从 ../../persistence/pg-pool.ts 导入；ScopeId 改从 ../memory/types.ts 导入。
 */
